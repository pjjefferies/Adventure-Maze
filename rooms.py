...
class room(object):
    def __init__(self,
                 ...
                 doors=None,  # [N,E,S,W] list of doors with values
                              # of next room, -3=no room, -1=start, -2 = exit
    ...
    def room_map(self):
        ...
        # Top/North Wall
        mid_wall_char = (self.door_char if self.doors[0] != self.no_door else
        ...
        for wall_piece in range(self.door_window_length):
            temp_map.append((self.door_char if self.doors[3] != self.no_door
                             else self.window_char if self.windows[3] else '|')
                            + ' ' * (self.wall_width * 2
                                     + self.door_window_width - 2)
                            + (self.door_char if self.doors[1] != self.no_door
                               else self.window_char if self.windows[1]
                               else '|'))
        ...
        # Bottom/South Wall
        mid_wall_char = (self.door_char if self.doors[2] != self.no_door else
                         (self.window_char if self.windows[2] != self.no_door
                          else '-'))
        temp_map.append('└' + '-' * (self.wall_width - 1)
                        + mid_wall_char * self.door_window_width
                        + '-' * (self.wall_width - 1) + '┘')
